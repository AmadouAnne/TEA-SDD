...
void print_BiTree(BiTree node, int level) {
    if (node == NULL) return;
    print_BiTree(node->right, level + 1);
    printf("%*s%d\n", 4 * level, "", *(int *)node->data);
    print_BiTree(node->left, level + 1);
}
...
static void on_display_BiTree_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (racine == NULL) {
        ...
        char BiTree_structure[1024] = "Structure de l'arbre :\n";
        print_BiTree(racine, 0); // afficher l'arbre dans la console
        gtk_label_set_text(label, BiTree_structure);
    }
}
...
static void on_display_balances_button_clicked(GtkButton *button, gpointer user_data) {
    ...
    if (racine == NULL) {
        ...
        BiTree_in_order(racine, monPrintF, NULL); // afficher les balances dans la console
    }
}
...
int main(int argc, char *argv[]) {
    ...
    gtk_window_set_title(GTK_WINDOW(window), "Menu Rouge Noir");
    ...
    GtkWidget *display_BiTree_button = gtk_button_new_with_label("Afficher la structure de l'arbre");
    gtk_box_pack_start(GTK_BOX(vbox), display_BiTree_button, FALSE, FALSE, 0);
    ...
    GtkWidget *label = gtk_label_new("Bienvenue dans le menu Rouge Noir");
    ...
    g_signal_connect(display_BiTree_button, "clicked", G_CALLBACK(on_display_BiTree_button_clicked), label);
